...
namespace HashBackCoreTests
{
    ...
    public class AuthHeaderBuilderTests
    {
        ...
        public async Task BuildAuthorization_KnownInput_ProducesExpectedBase64AndHash()
        {
            ...
            var (actualToken, actualHash) = HashBackBuilder.Build(host, now, unus, verify);

            /* Compare results. */
            Assert.AreEqual(expectedBase64, actualToken, 
        ...
        public async Task BuildAuthorization_DifferentInputs_ProduceDifferentHashes()
        {
            ...
            {
                var (token, hash) = HashBackBuilder.Build(host, $"{verify}_{counter}");
                hashes.Add(hash);
                tokens.Add(token);
            }
