...
/// <summary>Extension methods configuring the common <see cref="HashBackPolicy"/> rules, so
/// most callers never need to write their own delegates by hand.</summary>
public static class HashBackPolicyExtensions
{
    /// <summary>Configures the policy to only accept the supplied string as the request's Host value.</summary>
    ...
    /// <summary>Configures the policy to accept any of the supplied strings as the request's Host value.</summary>
    ...
    /// <summary>Configures the policy to only accept a Now value within the given number of
    /// seconds either side of the time returned by nowGetter, called fresh on each check.</summary>
    ...
    /// <summary>Configures the policy to only accept a Now value within the given number of
    /// seconds either side of the system clock (DateTime.UtcNow).</summary>
    public static void RequireNowWindow(this HashBackPolicy policy,
        int maxSecondsPast, int maxSecondsFuture)
    {
        policy.RequireNowWindow(
    ...
    /// <summary>Configures the policy to only accept a Now value within maxSeconds either
    /// side of the system clock (DateTime.UtcNow).</summary>
    ...
    /// <summary>Configures the policy to only accept a Now value within maxSeconds either
    /// side of the time returned by nowGetter, called fresh on each check.</summary>
